...
- **`JWT_SECRET`** (Railway env var) is mandatory in production — the server refuses to start without it rather than falling back to the dev secret.
- **`OPS_API_KEY`** (Railway env var) gates all ops/back-office routes (`/ops/*`, `/admin/fulfillment*`, `/admin/directory`, `/admin/media*`, `/admin/analytics/*`, `/admin/support/*`, `/admin/billing/*`, `/api/v1/premium/*`, plus the media-cleanup and Lambda-webhook endpoints). Callers must send `x-ops-key: <key>` (the `/ops/ws` WebSocket uses `?key=`). **When unset the guard is disabled** so local dev and the ops dashboard work with zero config — set it in production, and configure the ops dashboard to send the header when pointing at prod.
