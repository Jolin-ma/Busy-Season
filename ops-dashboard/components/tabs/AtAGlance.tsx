...
// ── MRR calc from billing ─────────────────────────────────────────────────────
const MRR = BILLING_ACCOUNTS.reduce((sum, a) =>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            label: 'MRR',
            value: `$${MRR.toFixed(0)}`,
            sub: `${BILLING_ACCOUNTS.filter(a => a.plan === 'PREMIUM').length} Premium · ${BILLING_ACCOUNTS.filter(a => a.plan === 'BASIC').length} Basic`,
            subOk: true,
