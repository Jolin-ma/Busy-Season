...
> **Trigger to watch — this decision expires on its own.** Registration becomes mandatory once taxable revenue passes **$30,000 over four consecutive calendar quarters**, and at that point the studio must start charging HST. That's ~19 Starter batches or ~1.7 Growth clients running a full year, so it is a plausible year-one outcome rather than a distant hypothetical. When it happens, the "no tax is added" copy becomes false and must come off the site the same week — and the pricing decision behind it has to be made deliberately: either list prices rise ~13% (a $400 batch invoices at $452) or the studio absorbs the tax and takes the margin hit. Deciding that in advance is much easier than renegotiating with signed Growth clients mid-retainer. Confirm the specifics with an accountant rather than relying on this note.
...
- [ ] **Monitor revenue against the $30,000 four-quarter GST/HST registration threshold** (Section 2). Crossing it forces registration, makes the site's "no tax is added" line false, and requires deciding in advance whether list prices rise ~13% or the studio absorbs it. Set the decision before the threshold is near, not after a Growth client has signed
